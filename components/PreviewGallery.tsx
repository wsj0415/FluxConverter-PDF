    ...
    <div className="w-full h-full overflow-y-auto pr-2 custom-scrollbar">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-4">
        {pages.map((page) => {
          ...
          return (
            <div 
              ...
              className={`
                group relative aspect-[3/4] rounded-lg bg-charcoal border overflow-hidden transition-all duration-200 select-none
                ${isInteractive ? 'cursor-pointer' : ''}
                ${page.status === 'error' ? 'border-red-500/30' : 
                  isSelected ? 'border-amber-500 ring-1 ring-amber-500/50' : 'border-white/10 hover:border-white/30 opacity-60 hover:opacity-100'}
              `}
            >
              {page.status === 'processing' ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-red-900/10 p-3 text-center">
                  ...
                  <button 
                      ...
                      className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-xs rounded border border-red-500/20 transition-all hover:scale-105 flex items-center gap-1.5 font-bold"
                  >
                ...
                <>
                  ...
                  <div className={`
                    ...
                    ${isSelected ? 'bg-amber-500 text-black scale-100' : 'bg-black/50 border border-white/20 text-transparent scale-90 group-hover:scale-100'}
                  `}>
              ...
              <div className={`absolute bottom-0 inset-x-0 backdrop-blur-sm p-2 text-center border-t transition-colors ${
                ...
                page.status === 'error' ? 'bg-red-950/50 border-red-500/20' : 'bg-black/60 border-white/5'
              }`}>
                <span className={`text-xs font-mono ${
                  page.status === 'error' ? 'text-red-400' : 
                  isSelected ? 'text-amber-400 font-bold' : 'text-gray-500'
                }`}>
        ...
        {skeletons.map((_, i) => (
          <div 
            key={`skel-${i}`} 
            className="aspect-[3/4] rounded-lg bg-charcoal/50 border border-white/5 flex items-center justify-center relative overflow-hidden"
          >
             <div className="absolute inset-0 bg-gradient-to-t from-transparent via-white/5 to-transparent animate-scan opacity-20" />
             <span className="font-mono text-xs text-gray-700">Waiting...</span>
          </div>
        ))}
      </div>
    </div>
